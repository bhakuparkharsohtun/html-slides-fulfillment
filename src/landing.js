 ...
 *
 * Both work/school accounts and personal Microsoft accounts are accepted.
 * The "common" authority admits both. What differs is how the entitlement is
 * keyed afterwards:
 *
 *   work      -> tenant:<tid>   one subscription covers the organisation
 *   personal  -> user:<oid>     one subscription covers one individual
 *
 * The personal case must never be keyed by tenant, because every consumer
 * account in the world reports the same tid.
 */
...
export async function handleAuthCallback(request, env) {
  ...
  const isWork = account.kind === "work";
  const pill = isWork ? "Organisation subscription" : "Personal subscription";
  const scope = isWork
    ? "Everyone in your organisation can use the add-in in PowerPoint."
    : "Your account can use the add-in in PowerPoint.";

  const ownerRow = isWork
