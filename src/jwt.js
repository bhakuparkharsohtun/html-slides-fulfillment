...
/* The tenant that issues tokens for personal Microsoft accounts. Anything
   presenting this tenant is a consumer account, not a work account. */
...
/**
 * Decide whether a set of claims represents a Microsoft Entra work or school
 * account. Personal Microsoft accounts are rejected.
 */
export function isWorkAccount(claims) {
  if (!claims || !claims.tid) return false;
  if (claims.tid === MSA_TENANT) return false;
  if (claims.idp === "live.com") return false;

  /* Personal accounts federated into a tenant surface as MSA in these claims. */
  const upn = String(claims.upn || claims.preferred_username || "");
  if (claims.idp && String(claims.idp).includes("live.com")) return false;

  return { tid: claims.tid, upn };
}
